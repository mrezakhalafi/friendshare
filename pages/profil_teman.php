...
<div id="cari2">
  ...
  <div id="kiri">
    ...
    <a href="teman.php?username=<?= $row['username']?>"><button id="tombol_navigasi_samping">Teman</button></a><br /><br />
        <a href="profil_teman_foto.php?username=<?= $row['username']?>"><button id="tombol_navigasi_samping">Foto</button><br /><br /></a>
  </div>
  ...
  <div id="tengah">
    <div id="buat_postingan">
      ...
      <img id="foto_postingan_awal" src="../<?= $row5['foto'] ?>" >
      ...
      <form id="form3" action="../proses/postingan/insert.php" method="post" enctype="multipart/form-data">
        ...
        <input type="hidden" name="nama" value="<?php echo $row5['nama']?>";>
        ...
        <input type="hidden" name="profil" value="<?php echo $row5['foto']?>">
        <input type="hidden" name="page" value="<?php echo basename($_SERVER['PHP_SELF'])?>">
        <input type="hidden" name="username" value="<?= $row5['username']?>">
...
Your browser does not support the video tag.
       ...
       <?php  
        ...
        <form id="form2" action="../proses/postingan/like.php" method="post" enctype="multipart/form-data">
          ...
          <input type="hidden" name="nama" value="<?= $row5['nama']?>">
          <input type="hidden" name="post" value="<?= $row2['id'] ?>">
          <input type="hidden" name="nama2" value="<?= $row2['username']?>">
                    <input type="hidden" name="username" value="<?= $row5['username']?>">
           <input type="hidden" name="page" value="<?php echo basename($_SERVER['PHP_SELF'])?>">
          <?php 
          $id = $row2['id'];
...
<?php endif; ?>
      ...
      <form id="form2" action="../proses/komentar/insert.php" method="post" enctype="multipart/form-data">
        ...
        <input type="hidden" name="profil" value="<?php echo $row5['foto']?>">
        <input type="hidden" name="page" value="<?php echo basename($_SERVER['PHP_SELF'])?>">
                <input type="hidden" name="username" value="<?php echo $row5['username']?>">
                <input type="hidden" name="nama2" value="<?= $row2['username']?>">
